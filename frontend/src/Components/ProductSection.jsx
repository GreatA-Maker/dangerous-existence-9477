...
import { Box, Checkbox, Text,Flex, SimpleGrid, Stack,Link,HStack,Select } from "@chakra-ui/react";
...
const ProductSection = () => {
  ...
  <>
 {
   ...
   <Box borderBottom="1px solid grey">
      <Flex className={Styles.sort}>
       ...
       <Box textAlign={"left"} className={Styles.sort}>
        <Text>Men T-Shirts - 80 items</Text></Box>
      ...
      <Box>
      <Select placeholder='Sort by : Recommended' value={orderBy} variant='outline' onChange={handleChangePrice}>
  ...
  <Flex>
  <Box textAlign={"left"} border="1px solid grey" w={["40%","30%","30%","15%"]} p="0px 10px 15px 15px">
  ...
  <Flex mt="10px" flexDirection={"column"} borderTop="1px solid grey">
    ...
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"Roadster")}>Roadster</Checkbox>        
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"Puma")}>Puma</Checkbox>
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"WROGN")}>WROGN</Checkbox>
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"Levis")}>Levis</Checkbox>
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"HRX")}>HRX</Checkbox>  
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"Moda Rapido")}>Moda Rapido</Checkbox>
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"Tommy Hilfiger")}>Tommy Hilfiger</Checkbox>
    <Checkbox onChange={()=>setFilterBy(filterBy?"":"ADIDAS")}>ADIDAS</Checkbox>
  </Flex>
 ...
 <Box className={Styles.box} ml={["20px","40px","8px","50px"]}>
  <SimpleGrid className={Styles.grid} columns={[1,2,3,5]} spacing={8}>
        {
          data?.map((item) => {
            return (
              <Box key={item.id}>
          ...
          <Box textAlign={"center"} width={["100%","100%","50%","50%"]} m="auto" mt="30px">
             <Pagination 
